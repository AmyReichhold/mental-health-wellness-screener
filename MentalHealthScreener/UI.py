...
Credits: User Akash Shendage for his post on how to make a scrollbar using
         ...
         https://stackoverflow.com/questions/19860047/python-tkinter
         -scrollbar-for-entire-window
"""
...
# Large text blocks for various instructions and message in the screener.
...
INSTRUCTIONS = "\nFor each item, please mark the box for Not True, Somewhat " \
               ...
               "" \
               "as best you can even if you are not absolutely certain . " \
               "Please " \
               "give your answers on the basis of how things have been for " \
               "you " \
               "over the last six months.\n"
INSTRUCTIONS2 = "These instructions will be provided on the next page as well " \
                "for" \
                " your convenience.\n"
WARNING = "This screener is only for personal use. Do NOT share the results" \
          ...
          "be respected.\n"
SOURCES = "For more information on the SDQ please visit: " \
          "https://sdqinfo.org.\n" \
          "To learn more about this project please visit the documentation " \
          "listed on the github repo at " \
          "https://github.com/lvandeca/CIS422Project2-Screener"
FINAL_MESSAGE = "You have completed the SDQ mental health screener!\n\nTo " \
                "view your results, you will" \
                " find a file labeled 'results.txt' " \
                "in the same folder as this application that will contain a " \
                "complete " \
                "summary. Please remember, these results are intended for " \
                "personal viewing, " \
                "or with the help of a psychologist.\n\nThank you for taking " \
                "the SQD! Please select 'Take Again' " \
                "to take the screener again or select 'Finish' to close the " \
                "application."
...
class UI():
    ...
    def display_survey(self):
        ...
        x_scrollbar = ttk.Scrollbar(sec, orient=HORIZONTAL,
                                    command=my_canvas.xview)
        x_scrollbar.pack(side=BOTTOM, fill=X)
        y_scrollbar = ttk.Scrollbar(main_frame, orient=VERTICAL,
        ...
        title = Label(border, text=TITLE,
                      font=("Times New Roman", 28, "bold")).pack()
        ...
        for question in self.screener.questions:
            qtext = Label(questionsDisplay, text=question.message,
                          font=("Times New Roman", 18)).grid(sticky=W,
                                                             row=rowtrack,
                                                             column=0)
            # Radio_answer is tied to the value given from the radio button
            # objects
            Radio_answer = IntVar()
            for opt in range(3):
                new_option = Radiobutton(questionsDisplay, value=opt,
                                         variable=Radio_answer).grid(
                    row=rowtrack,
                    column=opt + 1)
            # each question keeps track of the state of it's answer, once
            # the submit button is pressed, the question sets it's value
            # equal to its state
    ...
    def final_window(self):
        ...
        repeat = Button(buttons, text="Take Again",
                        font=("Times New Roman", 20),
                        command=lambda: {self.final.destroy(),
                                         self.init_interface()},
